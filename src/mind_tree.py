...
TO DO:
* Each trial its own decision tree
@author: joram
"""
...
from scipy.sparse.csgraph import johnson, dijkstra # global distances
...
class trial_ensemble():
    """ An ensemble per trial """
    

class ensemble():
    """ Ensemble of trees """    
    
    def __init__(self, x, manifold_dim=None, n_trees=1, seed=123, P=None):
        """ Give P as input to start from precomputed transition probs """
        self.x = x
    ...
    def compute_distances(self):
        ...
        if self.P is None:
            self.compute_probs()
        D = np.ones_like(self.P) 
        D[self.P>0] = np.sqrt(-np.log(self.P[self.P>0]))
        D[self.P<=0] = np.max(D[self.P>0])*1000
        self.D_local = D #(D+D.T)/2
        D = johnson(D) # global distances
        #D += np.eye(D.shape[0]) * np.diag(self.D_local)
        self.D = (D + D.T)/2 # symmetrize
        
    def loss_fun(self, y):
        """ Loss function for learning intrinsic coordinates y
            y: candidate solution (n, dim) """
        if self.D is None:
    ...
    def learn_coordinates(self, init='MDS'):
        """ Learn intrinsic coordinates that minimize self.loss_fun """
        ...
        if init == 'MDS':
            ...
            print("Random init")
            y_init = np.random.normal(size=(self.x.shape[0], self.manifold_dim))
        self.y_init = y_init
        loss_grad = grad(self.loss_fun)
        print("Perform optimization")
        #res = minimize(self.loss_fun, x0=y_init.flatten(), jac=loss_grad)
        res = fmin_cg(self.loss_fun,x0=y_init.flatten(), fprime=loss_grad)
        #print(res)
        y_final = res.reshape(y_init.shape)#['x'].reshape((y_init.shape))
        # Rotate to obtain canonical representation
        pca = PCA()
        self.coordinates = pca.fit_transform(y_final)
        

class mind_tree():
    """ Implements a single tree"""    
    
    def __init__(self, x, manifold_dim=None, rng=None, seed=123):
        """ x: (T, N) with N neurons, T time 
            manifold_dim : dimensionality of the manifold
            rng : randomState """
        self.x = x
        if rng is None:
            self.rng = np.random.RandomState(seed)
    ...
    def fit_tree(self, x=None,level=0, number=0, n_dir=10, n_leaf=40):
        """ Recursively partition state space by fitting decision trees
            x : states (neurons, time points)
            level : counting from root (==0)
            number : of the node, depth first 
            n_dir: no. of random splitting directions
            n_leaf: minimum samples in a leaf node (stop criterion)
            start recursion by calling: fit_tree(level=0, number=0)"""
        if x is None:
            x = self.x
        n = x.shape[0]  # samples
        # Done when no. of samples below threshold
        ...
        # Compute score (log like) of every split
        scores  = np.ones((n_dir,))*inf
        for i in range(n_dir):        
            ...
            # but only when candidate contains enough samples
            if p*n > n_leaf and (1-p)*n > n_leaf:
                L = x[successor_idx]
                R = x[successor_idx]
                # score: weighted average of log likes single splits
                err_L = np.mean((L - L.mean(axis=0))**2)
                err_R = np.mean((R-R.mean(axis=0))**2)
                scores[i] = err_L + err_R #self.fit_ppca(L).ll*p + self.fit_ppca(R).ll*(1-p)
        # select best split
        i_opt = np.argmin(scores)
        v_opt = v[:,i_opt] 
        c_opt = c[i_opt] 
        idx_left = np.dot(x, v_opt) < c_opt
        # Done when no. of samples in optimal split below threshold
        ...
        if (level==0) & (number==0): 
            # tracked back to root node, return first split as tree
            self.tree = split
        return split
    ...
    def forward_mapping(self, x_new, k=3, c=0.01):
        """ Find intrinsic coordinates of original coordinates x_new
            using NNs of x_new
    ...
    def backward_mapping(self, y_new, k=3, c=0.01):
        """ Find original coordinates of intrinsic coordinates y_new 
            using NNs of y_new
            k: no. of NNs, c: regularization """
    ...
    def get_prob(self, x_old, x_new):
        ...
        pca = node['pca']
        mu = pca.mean_
        C = pca.get_covariance() + 0.1*np.eye(pca.mean_.shape[0])
        #C *= np.eye(self.dim) + 0.01*np.eye(self.dim)#extract diag. ?
    ...
    def compute_distances(self):
        ...
        if self.P is None:
            self.compute_probs()
        D = np.ones_like(self.P) * 100
        D[self.P>0] = np.sqrt(-np.log(self.P[self.P>0]))
        self.D_local = D.copy() #(D+D.T)/2
        D = dijkstra(D) # global distances
        #D += np.eye(D.shape[0]) * np.diag(self.D_local)
        self.D = (D + D.T)/2 # symmetrize
